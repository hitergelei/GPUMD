...
# 注意param中，rc和h是公有的，所以在dipole和quadrupole中都要保持一致
dipole_param = \
{'Mo':
    {
    'd1': -0.10194129,
...
# vectorlized version of emb(rho)  (用于自动微分求导 via hjchen)
def emb_vec(rho):
    rho_e = para['rhoe']
    Fn0 = para['Fn0']
    Fn1 = para['Fn1']
    ...
    # 计算每个区间的条件
    condition_1 = rho < rho_n
    condition_2 = (rho >= rho_n) & (rho < rho_0)
    condition_3 = rho >= rho_0

    # 初始化结果数组
    result = np.zeros_like(rho)

    # 当 rho < rho_n 时
    x1 = (rho[condition_1] / rho_n - 1)
    x2 = x1**2
    x3 = x1**3
    e10 = Fn0
    e11 = Fn1 * x1
    ...
    # 当 rho >= rho_0 时
    x = rho[condition_3] / rho_s
    lnx = np.log(x)
    result[condition_3] = Fe * (1 - eta * lnx) * (x)**eta
...
def rho(r):
    ...
    term2 = 1 + np.power((r / re - lambda_), 20)   # term2 = 1 + (r / re - lambda_)**20  
    return term1 / term2
...
def phi_AA(r):
    ...
    # 使用np.power()速度更快，且支持传入为数组的参数
    term2 = 1 + np.power((r / re - kappa), 20)   # 1 + (r / re - kappa)**20
    left = term1 / term2

    term3 = B * np.exp(-beta * (r / re - 1))
    term4 = 1 + np.power((r / re - lambda_), 20)  # 1 + (r/ re - lambda_)**20
    right = term3 / term4
    return left - right
...
def psi(x):
    # 使用NumPy操作重写函数，适用于x为数组的输入
    return np.where(x < 0, x**4 / (1 + x**4), 0)
