...
# 源代码版本：https://github.com/qzhu2017/PyXtal_FF/blob/master/pyxtal_ff/calculator/lammpslib.py
# 其他版本：https://github.com/libAtoms/pymatnest/blob/master/lammpslib.py
...
class LAMMPSlib(Calculator):
    ...
    def calculate(self, atoms):
        ...
        # print("self.lmp.extract_box() = ", self.lmp.extract_box())
        # boxlo,boxhi,xy,yz,xz,periodicity,box_change = self.lmp.extract_box()
        box = self.lmp.extract_box()
        print("box = ", box)
        # extract_global用法 :https://matsci.org/t/accessing-boundary-dimensions-in-python/32543/2
        # https://docs.lammps.org/Python_module.html
        # https://www.afs.enea.it/software/lammps/doc19/html/Python_library.html
        # xlo = box[0][0] # self.lmp.extract_global("boxxlo", 1)   # 等价写法print(self.lmp.extract_box()[0][0])
        # print("xlo = ", xlo)  
        # xhi = box[1][0]  # 等价写法print(self.lmp.extract_box()[1][0])
        # print("xhi = ", xhi)

        xlo, ylo, zlo = box[0][0], box[0][1], box[0][2]
        xhi, yhi, zhi = box[1][0], box[1][1], box[1][2] 
        xy = box[2]
        yz = box[3]
        xz = box[4]
...
if __name__ == '__main__':
    ...
    #struc = bulk('Al', 'fcc', cubic=True)

    # 聚泰: /home/hjchen/projects/al_empirical_potentials/tests/Mo_adp_Baysian_opt/datasets/Baysian_OPT_ADP_based_DFT_Energy
    # struc = read("../lammps-data/Mo_structs_0.dat")   
    # struc = read("Mo_structs_0.dat", format="lammps-data")  # For Mo
    # print("---------1-----struc = ", struc)
    # struc.set_tags([1]*len(struc))
    # print("---------2-----struc = ", struc)
    # parameters = ["mass              1 42",
    #               "pair_style  adp",
    #               "pair_coeff * * ./Mo.adp Mo"
    #               ]
    struc = read("Mo_structs_47.dat", format="lammps-data")
    print("---------1-----struc = ", struc)
    struc.set_tags([1]*len(struc))
    print("---------2-----struc = ", struc)
    parameters = ["mass              1 42",
    ...
    # print(np.sum(struc.get_forces(), axis=0))
    print('stress: ')
